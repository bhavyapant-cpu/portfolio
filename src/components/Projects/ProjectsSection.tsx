...
export const ProjectsSection: React.FC = () => {
  ...
  return (
    <section id="projects-section" className="py-24 px-6 sm:px-12 md:px-20 bg-dark-950 relative border-t border-white/5">
      {/* Background Lighting */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-accent-blue/5 rounded-full blur-[160px] pointer-events-none" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {projectsData.map((project, idx) => (
            <motion.div
              ...
              className="group relative rounded-3xl p-8 sm:p-10 glass-card glass-panel-hover cursor-pointer overflow-hidden flex flex-col justify-between"
            >
              {/* Specular Highlight */}
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <span className="px-3.5 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs font-mono text-accent-cyan">
                    {project.category}
                  </span>
                ...
                <div className="space-y-2">
                  <h3 className="text-2xl sm:text-3xl font-bold text-white group-hover:text-accent-cyan transition-colors">
                    {project.title}
                  </h3>
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  {project.metrics.slice(0, 2).map((m, mIdx) => (
                    <div key={mIdx} className="p-3.5 rounded-xl bg-dark-900/80 border border-white/10 space-y-1">
              ...
              {/* Bottom Trigger Bar */}
              <div className="mt-8 pt-4 border-t border-white/10 flex items-center justify-between text-xs font-mono text-slate-400 group-hover:text-white">
                <span>Click to view interactive diagram</span>
                <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-accent-cyan group-hover:text-dark-950 transition-colors">
