...
export const SkillsVisualization: React.FC = () => {
  ...
  return (
    <section id="skills-section" className="py-24 px-6 sm:px-12 md:px-20 bg-dark-950 relative border-t border-white/5">
      {/* Glow Backdrop */}
      <div className="absolute top-1/2 right-1/4 w-[500px] h-[500px] bg-accent-cyan/5 rounded-full blur-[180px] pointer-events-none" />
        ...
        <div className="flex flex-wrap gap-2 p-1.5 rounded-2xl bg-white/5 border border-white/10 w-fit">
          {categories.map((cat) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-xs font-mono transition-all duration-300 ${
          ...
          <AnimatePresence>
            {filteredSkills.map((skill) => (
              <motion.div
                ...
                className="group p-6 rounded-2xl glass-card hover:border-accent-cyan/30 transition-all duration-300 space-y-4 flex flex-col justify-between"
              >
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    {skill.highlightBadge && (
                      <span className="px-2.5 py-0.5 rounded-full bg-accent-cyan/10 border border-accent-cyan/30 text-accent-cyan text-[10px] font-mono">
                        {skill.highlightBadge}
                      </span>
                    )}
                  </div>
